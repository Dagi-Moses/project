...
import 'package:project/src/services/fetch_IMEI.dart';
import 'package:project/src/ui/constants/colors.dart';
import 'package:project/src/ui/pages/homepage.dart';
import 'package:project/src/ui/widgets/Info_dialog_box.dart';
import 'package:project/src/ui/widgets/loader_dialog.dart';
...
class _LoginState extends State<Login> {
  ...
  void validateAndSubmit() async {
    DocumentReference? _empIdRef =
        db.collection('employeeID').doc(_username.toString());
    FocusScope.of(context).unfocus();
    onLoadingDialog(context);
    String email;
    try {
      DocumentSnapshot snapshot = await _empIdRef.get();
  ...
  /* Future<List> checkForSingleSignOn(User _user) async {
    DataSnapshot dataSnapshot =
        (await _userRef.child(_widget.user.uid).once()).snapshot;

    if (dataSnapshot != null) {
      var widget.user.uid = (dataSnapshot.value as Map)["widget.user.uid"];
      List listOfDetails = await getDeviceDetails();

      if (widget.user.uid != null) {
        if (listOfDetails[2] == widget.user.uid)
  ...
  loginUser(String email) async {
    if (_password != null) {
      try {
        _user = await signIn(email, _password!);

        // checkForSingleSignOn(_user).then((list) {
        //   Navigator.of(context).pop();
        //
        //   // Adding widget.user.uid to database
        //   if (list[0] == true && list[2] == false) {
        //     _userRef.child(_widget.user.uid).update({"widget.user.uid": list[1]});
        //   }
        //
        //   if (list[0] == true) {
